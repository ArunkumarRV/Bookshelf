...
System.out.println("iddddu"+userid);
session.setAttribute("prouserid",userid);
String order=request.getParameter("uid");
System.out.println("idddduid"+userid);

session.setAttribute("proorder",order);
...
<div class="container">
	<div class="center">
		<div class="payment-box">
			...
			<form class="form" action="proceedtoorder.jsp">
				...
				<input class="card-holder icon" type="text"  name="name" required placeholder="Cardholder Name">
<%-- <% 
String email= session.getAttribute("email").toString();
Connection con=dbconn.create();
Statement st=con.createStatement();
	ResultSet rs=st.executeQuery("SELECT * FROM `bookshop`.`money`  where name='"+pname+"' ");
	
	while(rs.next())
		
	{
		
		a=Integer.valueOf(quantity);
		b=Integer.valueOf(rs.getString(9));
		c=b-a;
		Connection con1=dbconn.create();
		Statement st1=con1.createStatement();
		st1.executeUpdate("UPDATE bookshop.money m SET quantity='"+c+"' where name='"+pname+"'");

		 st1.executeUpdate("UPDATE bookshop.card m SET Address='"+address+"' where productname='"+pname+"' and uemail='"+email+"'   "); 
...
%>
<input type="hidden" name="rand" value="<%=rand_int1%>"> 
